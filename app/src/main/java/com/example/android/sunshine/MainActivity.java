...
public class MainActivity extends AppCompatActivity {
    ...
    private void loadWeatherData() {
        ...
        URL weatherUrl = NetworkUtils.buildUrl(userPreferredLocation);
        // display the URL to make sure we built it right
    ...
    public class WeatherQueryTask extends AsyncTask<URL, Void, String> {
        ...
        protected String doInBackground(URL... params) {
            ...
            String weatherQueryResults = null;
            try {
                weatherQueryResults = NetworkUtils.getResponseFromHttpUrl(weatherUrl);
            } catch (IOException e) {
                e.printStackTrace();
            }
            return weatherQueryResults;
        }

        // COMPLETED (7) Override the onPostExecute method to display the results of the network request
        @Override
        protected void onPostExecute(String weatherQueryResults) {
            //mLoadingIndicator.setVisibility(View.INVISIBLE);
            if (weatherQueryResults != null && !weatherQueryResults.equals("")) {
                //showJsonDataView();
                //mSearchResultsTextView.setText(githubSearchResults);
                mWeatherTextView.setText(weatherQueryResults);
            } else {
                //showErrorMessage();
            }
        }
